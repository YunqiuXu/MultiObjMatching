...
#                                                                             #
# This is version 3: surf on original size, do not use pyr                                    #
###############################################################################
...
def mapDown(objectes_or_bboxes):
    new_boxes = []
    for item in objectes_or_bboxes:
        item = [i / 2 for i in item]
        new_boxes.append(tuple(item))
    return new_boxes

# Remap the keypoints to original image
# input: kps, bboxes on pyramid
# output: kps on original image
def remap_kps(kps, bbox):
    xmin = bbox[0]
    ymin = bbox[1]
    for kp in kps:
        kp.pt = (kp.pt[0] + xmin, kp.pt[1] + ymin)
    return kps


# Remap the contour to original image
...
# Input: 
#   img_template, img_test: the crop of original size
#   object_template: the object position for img_template in original size
#   bbox_template, bbox_test: the bbox of original size
# Output:
#   kp1, kp2: keypoints
...
# Build the frame of img_match: combination of img1 and img2
...
if __name__ == "__main__":
    ...
    img_template_original = img_template.copy()
    objects, bboxes = get_all_objects_bboxes(img_template_copy) # in original size

    # cv2.imwrite("finished/model_frame.jpg", img_template_original)
    ...
    img_template_draw = img_template.copy() # original size after drawing obj and bboxes

    # cv2.imwrite("finished/model_frame_after_drawing_bbox.jpg", img_template)
    ...
    # small_bboxes = mapDown(bboxes)
    # print "Map down bboxes : " + str(small_bboxes)
    ...
    # Return img_template in original size, but sharpened
    ...
    while camera.isOpened():
        ...
        # if count % 100 == 0:
        #     cv2.imwrite("finished/img_test_original_" + str(count) + ".jpg", img_test_original)
        ...
        for i in range(len(objects)):
            ...
            # Get test bbox
            object_template = objects[i] # in original size
            bbox_template = bboxes[i] # in original size
            _, bbox_test = trackers[i].update(image) # small size
            # bbox_test = [2 * x for x in bbox_test] # remap to original size
            

            # Get crop on original img
            img_template_crop = crop_bbox(img_template_original, bbox_template)
            img_test_crop = crop_bbox(img_test_original, bbox_test)

            # if count % 100 == 0:
            #     cv2.imwrite("finished/model_frame_obj_" + str(i) + ".jpg", img_template_crop)
            #     cv2.imwrite("finished/img_test_" + str(count) + "_obj_" + str(i) + ".jpg", img_test_crop)
        ...
        ## [TEST]
#        if count == 100:
#            img_test_fuck = img_test_original.copy()
#            for i in range(len(objects)):
#                curr_bbox = all_bbox_test[i]
#                img_test_fuck = draw_bbox(img_test_fuck, curr_bbox, colorList[i])
#            cv2.imwrite("img_test_after_bbox.jpg", img_test_fuck)
        ...
        for i in range(len(objects)):
            ...
            curr_bbox = all_bbox_test[i]
            img_test_original = draw_bbox(img_test_original, curr_bbox, colorList[i])
            
            # get current trace centre
            trace_center = (int(curr_bbox[0] + curr_bbox[2] / 2), int(curr_bbox[1] + curr_bbox[3] / 2))
            if trace_center != (0,0):
...
#        if count % 100 == 0:
#            cv2.imwrite("finished/img_match_" + str(count) + ".jpg", img_match)
#        count += 1
