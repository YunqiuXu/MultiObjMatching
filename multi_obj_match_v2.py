...
# This is version 2: orb on original size                                     #
###############################################################################
...
def match_surf_flann(img_template, img_test, object_template, bbox_template, bbox_test):
    ...
    index_params = dict(algorithm = FLANN_INDEX_KDTREE, trees = 30) # larger tree will consume more storage but speed up the code
...
if __name__ == "__main__":
    ...
    if len(tracker_modes) == 1: # default KCF or only one tracker
        for i in range(len(bboxes)):
            ...
            tracker.init(img_template, small_bboxes[i])
    ...
    while camera.isOpened():
        ...
        for i in range(len(objects)):
            ...
            if len(all_good[i]) > 10: # threshold
